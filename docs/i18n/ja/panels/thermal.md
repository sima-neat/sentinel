...
Sentinel は Linux の `simaai_modalix_thermal_sensor` hwmon デバイスを読み取ります。`temp1_input`〜`temp14_input` は `rtsn_0`〜`rtsn_13` に対応し、ミリ度 Celsius の値を 1000 で割って表示します。測定モード、制御手順、校正はカーネルが管理します。

Sentinel は `/dev/mem` をマッピングせず、PVT コントローラーに書き込みません。直接アクセスはカーネルと競合するため、代替手段としても使用しません。hwmon ドライバーがない場合、チップ内温度は取得できません。カーネルの温度保護は有効のままにしてください。
...
センサーが存在しない場合や読み取りに失敗した場合は、取得不可（JSON `null`）として表示します。ゼロや前回の値は使用しません。次の収集時に再試行し、ドライバーの再登録にも対応するため hwmon デバイスを毎回再検出します。
